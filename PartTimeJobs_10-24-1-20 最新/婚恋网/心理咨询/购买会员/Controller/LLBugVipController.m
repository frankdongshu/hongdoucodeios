...
- (void)viewDidLoad {
    ...
    self.sc_navigationBar.title = @"购买会员";
...
- (void)didSelectBuyButtonWithProductID:(NSString *)productID {
    ...
    [_IAPTool startIAPWithProductID:productID completeHandle:^(IAPResultType type, NSData * _Nonnull data) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            switch (type) {
                ...
                case IAPResultCancle:
                    [self.view showErrorWithMessage:@"用户取消购买"];
                    break;
...
- (void)getMemberDate {
    ...
    [HTTPSessionManger postDataWithNSString:@"/coach/get_member" withDictionary:parmas success:^(NSDictionary * _Nonnull dictionary) {
        
        NSLog(@"$$%@",dictionary);
        
        if ([[dictionary[@"code"] stringValue] isEqualToString:@"200"]) {
            
            self.headerView.typeLabel.attributedText = [[NSString alloc] attriStringWithFirstString:@"您的会员到期日是：" withTwoString:[NSString stringWithFormat:@"%@",!kISNullObject(dictionary[@"data"][@"date"]) ? dictionary[@"data"][@"date"] : @"未开通"] withThreeString:@"" color:[UIColor blackColor]];
