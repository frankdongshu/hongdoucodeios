...
@interface HLPullBlackViewController ()<HLPrivacyManageDeleagte>
{
    UIAlertView * alert;
...
@implementation HLPullBlackViewController
...
-(void)initTableView
{
    ...
    lable.textColor = [UIColor colorWithHex:0x6175F6];
    lable.font = [UIFont systemFontOfSize:14];
    lable.text = @"被屏蔽的人互相之间不能看到资料以及发送消息";
    [self.view addSubview:lable];
    
    
    self.tableViewInsertTop = 0;
    self.tableView.contentInsetTop = 0;
    self.tableView.frame = CGRectMake(0, 20, kScreenWidth, kScreenHeight - kNavigationBarHeight - 20);
...
- (void)loadNewData{
    WeakSelf(weakSelf);
    [HLHTTPSessionManager postDataWithNSString:HLPrivacy_PullBlack withDictionary:@{@"uid":[LoginManager defaultManager].userid} success:^(NSDictionary * _Nonnull dictionary) {
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (buttonIndex == 1) {
        if (alertView.tag == 110){
            ...
            [HLHTTPSessionManager postDataWithNSString:HLDelegate_PullBlack withDictionary:@{@"uid":[LoginManager defaultManager].userid,@"bid":model.userid,@"mobile":model.nickname} success:^(NSDictionary * _Nonnull dictionary) {
