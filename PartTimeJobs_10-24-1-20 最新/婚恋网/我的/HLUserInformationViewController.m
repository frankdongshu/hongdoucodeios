...
#import "CSHeaderViewCell.h" // 设置头像
...
//    self.keyArry = @[@[@""],@[@"listen"],@[@"age",@"habitation",@"earns",@"height",@"weight",@"education",@"industry",@"housing",@"car",@"registered",@"native",@"nation",@"school",@"company",@"position",@"marital",@"children",@"blood"],@[@"age",@"habitation",@"income",@"height",@"education"]];
    ...
    self.userTitleArray = @[@[@"头像",@"昵称",@"居住地"]];
    self.keyArry = @[@[@"head",@"nickname",@"habitation"]];
...
//    _tableView.separatorStyle = UITableViewCellSeparatorStyleNone;
    ...
    [_tableView registerNib:[UINib nibWithNibName:@"CSHeaderViewCell" bundle:nil] forCellReuseIdentifier:@"CSHeaderViewCell"];
...
{
//    NSArray *array = @[@"名片相册(高质量照片有助于获得更多关注)",@"倾听我心",@"基本资料",@"交友条件"];
    NSArray *array = @[@"基本资料"];
...
//    }
    ...
    if (indexPath.row == 0) {
        ...
        NSString *headString = kISNullObject(self.userInfo.head)?@"":self.userInfo.head;
...
//    return cell;
    ...
    cell.textLabel.text = self.userTitleArray[indexPath.section][indexPath.row];
    
    if (indexPath.row == 1) {
        cell.detailTextLabel.text = self.userInfo.nickname;
    }
    if (indexPath.row == 2) {
        cell.detailTextLabel.text = self.userInfo.habitation;
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == 0) {
        ...
        if (indexPath.row == 2) { // 居住地
            ...
            citySelectVC.modalPresentationStyle = UIModalPresentationFullScreen;
        ...
        switch (indexPath.row) {
            ...
            {
                //车
//                [BRStringPickerView showStringPickerWithTitle:@"车" dataSource:@[@"未购",@"已购"] defaultSelValue:self.userInfo.car resultBlock:^(id selectValue) {
...
- (void)requestListWithUrl:(NSString *)url withTitle:(NSDictionary *)dic withCurrentSelect:(NSString *)selectTitle{
    ...
    [HLHTTPSessionManager getDataWithNSString:url withDictionary:@{} success:^(NSDictionary * _Nonnull dictionary) {
        if ([[[dictionary objectForKey:@"code"] stringValue] isEqualToString:@"200"]) {
            ...
            [BRStringPickerView showPickerWithTitle:dic[@"name"] dataSourceArr:allVaule selectIndex:index resultBlock:^(id selectValue) {
...
//            [BRStringPickerView showStringPickerWithTitle:dic[@"name"] dataSource:allVaule defaultSelValue:string resultBlock:^(id selectValue) {
//                for (NSString *key in allDiction.allKeys) {
//                    if ([allDiction[key] isEqualToString:selectValue]) {
//                        if (weakSelf.currentIndexPath.section==2) {
//                            [weakSelf uploadUserInfoVaule:key];
...
- (void)uploadUserInfoVaule:(NSString *)vaule{
    ...
    WeakSelf(weakSelf);
    [HLHTTPSessionManager postDataWithNSString:HLEdit_UserModify withDictionary:@{@"uid":[LoginManager defaultManager].userid, @"type":self.keyArry[_currentIndexPath.section][_currentIndexPath.row],@"val":vaule} success:^(NSDictionary * _Nonnull dictionary) {
