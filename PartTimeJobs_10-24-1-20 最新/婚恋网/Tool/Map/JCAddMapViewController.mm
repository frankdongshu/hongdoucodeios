...
- (void)viewDidLoad {
    ...
    self.sc_navigationBar.title = @"位置";
...
- (void)loadMapView {
    annotaionShow = NO;
    if (_isOnlyShowMap) {
        mapView = [[BMKMapView alloc]initWithFrame:CGRectMake(0, kNavigationBarHeight, kScreenWidth, kScreenHeight)];
        
    } else {
        mapView = [[BMKMapView alloc]initWithFrame:CGRectMake(0, kNavigationBarHeight, kScreenWidth, kScreenHeight/2)];
        ...
        locationBtn.frame = CGRectMake(10, kScreenHeight/2-40, 30, 30);
        locationBtn.backgroundColor = [UIColor clearColor];
//        [locationBtn setTitle:@"定位" forState:UIControlStateNormal];
        [locationBtn setImage:[UIImage imageNamed:@"icon_weizhi"] forState:UIControlStateNormal];
