...
static const CGFloat FXFormFieldMinLabelWidth = 70;
...
- (void)viewDidLoad
{
    ...
    if (self.sc_navigationBar.leftBarButtonItem == nil) {
        
        HXBarButtonItem *leftBarItem = [[HXBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"navi_back"] style:HXBarButtonItemStylePlain handler:^(id sender) {
            
            [self.navigationController popViewControllerAnimated:YES];
        }];
        
        self.sc_navigationBar.leftBarButtonItem = leftBarItem;
    }
    
    if (!self.tableView)
    {
        CGRect frame = CGRectMake(0, kNavigationBarHeight,[UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height-kNavigationBarHeight);
...
- (UIView *)inputView
{
    _lastDate = self.datePicker.date;
...
- (UIView *)inputAccessoryView
{
    ...
    UIBarButtonItem *barButtonCancel = [[UIBarButtonItem alloc] initWithTitle:@"取消"
                                                                        style:UIBarButtonItemStyleBordered target:self action:@selector(cancleButtonTapped:)];
    UIBarButtonItem *flex = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemFlexibleSpace target:self action:nil];
    [toolBar setItems:[NSArray arrayWithObjects:barButtonCancel, flex, barButtonDone, nil]];
    return toolBar;
}
...
- (void)cancleButtonTapped:(UIBarButtonItem *)item
{
    if (_lastDate) {
        self.datePicker.date = _lastDate;
        [self valueChanged];
    }
    [self resignFirstResponder];
}
