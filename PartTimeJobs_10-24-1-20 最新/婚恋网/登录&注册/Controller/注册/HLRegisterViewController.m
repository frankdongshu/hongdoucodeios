...
- (void)createProtocol {
    ...
    _loginProtocol = ({
        ...
        [lawTitle setTextHighlightRange:[context rangeOfString:mianZe] color:REDColor backgroundColor:[UIColor clearColor] tapAction:^(UIView * _Nonnull containerView, NSAttributedString * _Nonnull text, NSRange range, CGRect rect) {
        ...
        [lawTitle setTextHighlightRange:[context rangeOfString:yinSi] color:REDColor backgroundColor:[UIColor clearColor] tapAction:^(UIView * _Nonnull containerView, NSAttributedString * _Nonnull text, NSRange range, CGRect rect) {
...
//    [_headerView az_setGradientBackgroundWithColors:@[[UIColor colorWithRed:189/255.0 green:100/255.0 blue:255/255.0 alpha:1.0],[UIColor colorWithRed:130/255.0 green:92/255.0 blue:244/255.0 alpha:1.0]] locations:@[@(0.0),@(0.7),@(1.0)] startPoint:CGPointMake(0, 0) endPoint:CGPointMake(1, 1)];
    ...
    [_phoneTextFile textFileTitle:@"phone" leftWidth:14 heigth:21];
    _phoneTextFile.delegate = self;
    _phoneTextFile.tag = 529;
    [_verityTextFile textFileTitle:@"mima" leftWidth:14 heigth:21];
    ...
    [_passwordTextFile textFileTitle:@"mima" leftWidth:14 heigth:21];
...
- (IBAction)sendVolitionClick:(id)sender {
    ...
    if (![NSString isValidateTelNumber:self.phoneTextFile.text])//没有通过正则表达式
    {
...
- (IBAction)registerClick:(id)sender {
    ...
    if (![NSString isValidateTelNumber:self.phoneTextFile.text]) {
        ...
        WeakSelf(weakSelf);
        [HLHTTPSessionManager postDataWithNSString:HLREGISTER withDictionary:@{@"mobile":self.phoneTextFile.text,@"sms":self.verityTextFile.text,@"device":[[UIDevice currentDevice] deviceModelName],@"password":self.passwordTextFile.text} success:^(NSDictionary * _Nonnull dictionary) {
...
//                //保存最后登录的手机号
//                [[LoginManager defaultManager] setAccount:weakSelf.loginView.phoneTextField.text];
                [[LoginManager defaultManager] setUserid:[dictionary[@"data"] objectForKey:@"id"]];
                [[LoginManager defaultManager] setAccount:[dictionary[@"data"] objectForKey:@"username"]]; // 用户名
                //保存最后登录的用户昵称
