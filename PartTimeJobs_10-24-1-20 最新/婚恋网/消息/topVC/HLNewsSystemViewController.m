...
@interface HLNewsSystemViewController ()
...
@implementation HLNewsSystemViewController
...
- (void)viewDidLoad {
    ...
    self.sc_navigationBar.title = @"系统消息";
    [self initTableView];
    [self loadNewData];
}
//创建tabbleview视图
-(void)initTableView
{
    ...
    [self.tableView registerNib:[UINib nibWithNibName:@"HLNewsSystemTableViewCell" bundle:nil] forCellReuseIdentifier:@"HLNewsSystemTableViewCell"];
...
- (void)loadNewData{
    WeakSelf(weakSelf);
    [HLHTTPSessionManager postDataWithNSString:HLFriends_System withDictionary:@{@"uid":[LoginManager defaultManager].userid} success:^(NSDictionary * _Nonnull dictionary) {
        NSString *code = [NSString stringWithFormat:@"%@",[dictionary objectForKey:@"code"]];
        if ([code isEqualToString:@"200"] ) {
            
//            weakSelf.dataSource = [XX mj_objectArrayWithKeyValuesArray:[dictionary objectForKey:@"data"]];
...
{
    HLNewsSystemTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"HLNewsSystemTableViewCell"];
