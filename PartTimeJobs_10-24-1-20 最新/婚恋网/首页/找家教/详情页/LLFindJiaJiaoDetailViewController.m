//
//  LLFindJiaJiaoDetailViewController.m
//  PartTimeJobs
//
...
@interface LLFindJiaJiaoDetailViewController ()<UITableViewDelegate, UITableViewDataSource, CoachHeadCellDeleagte>
...
@implementation LLFindJiaJiaoDetailViewController


- (UITableView *)tableView {
    if (!_tableView) {
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, kNavigationBarHeight, kScreenWidth, kScreenHeight-kTabBarHeight) style:UITableViewStyleGrouped];
...
//        lab.text = [NSString stringWithFormat:@"%@\t%@经验",self.model.identity,self.model.intelligence];
        lab.text = [NSString stringWithFormat:@"资料介绍"];
    } else if (section == 2) {
        lab.text = @"擅长领域";
    } else if (section == 3) {
        if (kISNullObject(self.model.motto)) {
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 2) { // 擅长领域
        CGFloat height ;
        if (self.model.curriculum.count==0) {
            return  0;
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) { // 头部视图
        ...
        cell.nameLab.text = [NSString stringWithFormat:@"%@/%@",self.model.nickname,self.model.sex];
        cell.cityLab.text = self.model.city;
        cell.sexLab.text = self.model.intelligence;
...
//            [self getComplaintList];
    ...
    self.bottomView = [[UIView alloc] initWithFrame:CGRectMake(0, kScreenHeight - kTabBarHeight- kTabBarHeight, kScreenWidth, kTabBarHeight)];
...
- (void)goChat{
    ...
    LLFindJiaJiaoDetailGoChatViewController * vc = [LLFindJiaJiaoDetailGoChatViewController new];
