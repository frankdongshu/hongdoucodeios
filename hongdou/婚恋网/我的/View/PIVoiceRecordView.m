...
@property (nonatomic, strong) UILabel *recordStateLabel; // 状态标签
@property (nonatomic, strong) UILabel *recordTimeLabel; //时间标签
@property (nonatomic, strong) UIImageView *recordImgView; // 录制视图
...
- (void)initViewsWithEnsureTitle:(NSString *)title
{
    ...
    //重录
    self.rerecordingBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    self.rerecordingBtn.hidden = YES;
    self.rerecordingBtn.backgroundColor = [UIColor whiteColor];
    self.rerecordingBtn.layer.cornerRadius = 67.0 /2  ;
//    self.rerecordingBtn.layer.borderWidth = 1;
//    self.rerecordingBtn.layer.borderColor = [UIColor colorWithHex:@"#999999"].CGColor;
    [self.rerecordingBtn setImage:[UIImage imageNamed:@"shanchu"] forState:UIControlStateNormal];
    ...
    @weakify(self);
    [[self.rerecordingBtn rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(id x) {
        @strongify(self);
        //重新录制
        [self reRecording];
    }];
    ...
    [reLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.right.equalTo(self.rerecordingBtn);
        make.top.equalTo(self.rerecordingBtn.mas_bottom).offset(12 );
    ...
    self.nextBtn.layer.cornerRadius = self.rerecordingBtn.layer.cornerRadius;
//    self.nextBtn.layer.borderWidth = 1;
//    self.nextBtn.layer.borderColor = self.rerecordingBtn.layer.borderColor;
    [self.nextBtn setImage:[UIImage imageNamed:@"queding"] forState:UIControlStateNormal];
    ...
    [nextLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.right.equalTo(self.nextBtn);
...
//            [self.rerecordingBtn setImage:[UIImage imageNamed:@"shanchu"] forState:UIControlStateNormal];
//            [self.nextBtn setImage:[UIImage imageNamed:@"queding"] forState:UIControlStateNormal];
//            self.rerecordingBtn.layer.borderColor = PrimaryColor.CGColor;
//            self.nextBtn.layer.borderColor = self.rerecordingBtn.layer.borderColor;
...
- (void)initVoiceStateViews
{
    ...
    //录音键
    self.recordImgView = [[UIImageView alloc] initWithFrame:CGRectZero];
    self.recordImgView.image = [UIImage imageNamed:@"huatong"];
    [self.readyView addSubview:self.recordImgView];
    
    [self.recordImgView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.equalTo(self.readyView);
        make.top.equalTo(self.readyView);
        make.width.height.equalTo(self.readyView);
    }];
    ...
    [recordLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.recordImgView.mas_bottom).offset(2.0 );
        make.left.right.equalTo(self.recordImgView);
        make.height.mas_equalTo(17.0 );
    }];
    
    // 录制状态标签
    self.recordStateLabel = [[UILabel alloc] initWithFrame:CGRectZero];
    self.recordStateLabel.text = @"按住说话, 点击录音";
    self.recordStateLabel.textColor = kRGBA(185, 185, 185, 1);
    self.recordStateLabel.font = [UIFont systemFontOfSize:15];
    self.recordStateLabel.textAlignment = NSTextAlignmentCenter;
    [self addSubview:self.recordStateLabel];
    
    [self.recordStateLabel mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    self.recordTimeLabel.textColor = [UIColor darkTextColor];
    self.recordTimeLabel.font = [UIFont boldSystemFontOfSize:30];
    ...
    [self.recordTimeLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.right.equalTo(self);
        make.bottom.equalTo(self.recordStateLabel.mas_top).offset(-15);
        make.height.mas_equalTo(25 );
    }];
    ...
    [self.pauseBtn setImage:[UIImage imageNamed:@"zanting"] forState:UIControlStateNormal];
    [self.pauseBtn setImage:[UIImage imageNamed:@"bofang"] forState:UIControlStateSelected];
    ...
    [self.pauseBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.width.height.equalTo(self.replayView);
    }];
    ...
    [replayBtn setImage:[UIImage imageNamed:@"bofang"] forState:UIControlStateNormal];
    ...
    [replayBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.finishView);
        make.centerX.equalTo(self.finishView);
        make.size.equalTo(self.finishView);
    }];
...
//                self.readyView.hidden = YES;
            ...
            case PIVoiceRecordViewStateReplaying: {
                ...
                self.recordStateLabel.text = @"正在播放";
...
//                self.readyView.hidden = YES;
    ...
    switch (self.state) {
        ...
        case PIVoiceRecordViewStateRecording: {
            //录制中
            self.recordStateLabel.text = @"正在录制";
            self.recordImgView.image = [UIImage imageNamed:@"luzhizhong"];
...
//            self.recordStateLabel.text = @"正在播放";
            ...
            self.recordTimeLabel.text = [NSString stringWithFormat:@"%lds",i];
        ...
        case PIVoiceRecordViewStateFinish: {
            self.recordStateLabel.text = @"录制完成";
...
- (void)pause:(BOOL)pause
{
    ...
    if (pause) {
        ...
        self.recordStateLabel.text = @"继续播放";
