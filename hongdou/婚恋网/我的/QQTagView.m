...
- (void)addTags:(NSArray *)tags
{
    ...
    for (int i = 0; i< tags.count; i++) {
        [self addLabel:tags[i][@"effect"] tag:[tags[i][@"id"] integerValue]];
    }
}

- (void)addFriendInfoTags:(NSArray *)tags
{
    for (int i = 0; i< tags.count; i++) {
...
- (void)setStyle:(QQTagStyle)Style {
    
    _Style = Style;
    
    for (int i = 0; i< self.dataArray.count; i++) {
        [self remove:self.dataArray[i][@"effect"]];
        [self addLabel:self.dataArray[i][@"effect"] tag:[self.dataArray[i][@"id"] integerValue]];
    }
}
