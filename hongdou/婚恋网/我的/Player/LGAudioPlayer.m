...
+ (id)shareInstance {
    static LGAudioPlayer *instance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[LGAudioPlayer alloc] init];
    });
    return instance;
}

- (void)setDelegate:(id<LGAudioPlayerDelegate>)delegate {
...
- (void)stopPlaying
{
    ...
    if ([self.delegate respondsToSelector:@selector(didAudioPlayerComplete)]) {
        [self.delegate didAudioPlayerComplete];
    }
    ...
    switch (status) {
        case AVPlayerItemStatusReadyToPlay: {
            ...
            if (self.startPlaying) {
                ...
                CGFloat f = [self audioDurationFromURL:self.url];
                
                self.startPlaying(AVPlayerItemStatusReadyToPlay, self.voiceItem ? f :0);
            }
        ...
        case AVPlayerItemStatusFailed: {
            ...
            if ([self.delegate respondsToSelector:@selector(didAudioPlayerStatusFailed)]) {
                [self.delegate didAudioPlayerStatusFailed];
            }
