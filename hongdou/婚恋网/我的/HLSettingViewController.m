...
- (void)viewDidLoad {
    ...
    isClose = YES;
    self.titleArray = @[@[@"通知设置",@"清除缓存",@"资料状态"],@[@"关于我们",@"防骗手册",@"隐私政策",@"免责声明",@"客服留言",@"注销"]];
    [self creatTableView];
...
//
//            // 通知广场刷新数据
//            [[NSNotificationCenter defaultCenter] postNotificationName:@"squareReloadList" object:nil];
    ...
    switch (indexPath.section) {
        case 0:
            if (row==1) {
                ...
                // 设置控件开启状态填充色
                swi.onTintColor = [UIColor colorWithHex:0x995ff8];
                [swi setOn:isClose];
    ...
    switch (indexPath.section) {
        case 0:
            if (row==0) {
                ...
                [self.view showLoadMessageAtCenter:@"清理中..."];
...
//                [self.navigationController pushViewController:leavVC animated:YES];
                ...
                // 创建弹窗
                    ...
                    UIAlertAction *okAction = [UIAlertAction actionWithTitle:@"确定"
                        ...
                        [HLHTTPSessionManager postDataWithNSString:HLlogout withDictionary:@{@"uid":[LoginManager defaultManager].userid} success:^(NSDictionary * _Nonnull dictionary) {
                            ...
                            [self.view showTostWithMessage:@"注销失败"];
