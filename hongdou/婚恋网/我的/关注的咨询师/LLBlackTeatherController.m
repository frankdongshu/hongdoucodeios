...
- (void)loadNewData{
    ...
    [HLHTTPSessionManager postDataWithNSString:@"/mind/myBlacklist" withDictionary:parmas success:^(NSDictionary * _Nonnull dictionary) {
        
        NSLog(@"/mind/myBlacklist: %@",dictionary);
        ...
        } else if ([[NSString stringWithFormat:@"%@",dictionary[@"code"]] isEqualToString:@"202"]) {
            // 没数据
...
- (void)requestCancelBlackWithRow:(NSInteger)row {
    ...
    [HLHTTPSessionManager postDataWithNSString:HLDelegate_PullBlack withDictionary:params success:^(NSDictionary * _Nonnull dictionary) {
        
        NSLog(@"/friends/delBlacklist: %@",dictionary);
