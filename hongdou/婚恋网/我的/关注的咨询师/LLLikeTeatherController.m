...
@interface LLLikeTeatherController ()
@property (nonatomic, strong)NSMutableArray *dataSource;

@end

@implementation LLLikeTeatherController
...
- (void)loadNewData{
    ...
    [HLHTTPSessionManager postDataWithNSString:@"/mind/myFollow" withDictionary:parmas success:^(NSDictionary * _Nonnull dictionary) {
        
        NSLog(@"/mind/myFollow: %@",dictionary);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.type = FollowType;
    cell.row = indexPath.row;
    cell.attentionBlock = ^(NSInteger row, NSInteger status) {
        NSLog(@"%ld",row);
...
- (void)showCancelFollowAlertWithRow:(NSInteger)row {
    ...
    UIAlertController *alerC = [UIAlertController alertControllerWithTitle:@"确认不再关注?" message:nil preferredStyle:UIAlertControllerStyleAlert];
    ...
    UIAlertAction *sure = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        [self requestCancelFollowWithRow:row];
    }];
...
- (void)requestCancelFollowWithRow:(NSInteger)row {
    ...
    [HLHTTPSessionManager postDataWithNSString:HLCancelFollow_Shields withDictionary:params success:^(NSDictionary * _Nonnull dictionary) {
