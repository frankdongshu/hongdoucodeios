//
//  HLShoppingExchangeController.m
//  hongdou
//
...
#import "HLShoppingExchangeController.h"
...
@interface HLShoppingExchangeController ()<UICollectionViewDelegate, UICollectionViewDataSource, HLProductListCellDelegate>{
...
@implementation HLShoppingExchangeController
...
- (void)viewDidLoad {
    ...
    self.sc_navigationBar.rightBarButtonItem= [[HXBarButtonItem alloc] initWithTitle:@"兑换记录" withColor:kRGBA(63, 70, 88, 1) style:HXBarButtonItemStylePlain handler:^(id sender) {
        @strongify(self)
        HLExchangeRecordController *recordVC = [[HLExchangeRecordController alloc] init];
        recordVC.hidesBottomBarWhenPushed = YES;
        [self.navigationController pushViewController:recordVC animated:YES];
    }];
    self.sc_navigationBar.title = @"商品兑换";
...
- (void)loadMoreData {
    ...
    currentPage ++;
    [self requestExchangeListWithPege:currentPage];
...
// 请求可兑换产品列表
- (void)requestExchangeListWithPege:(NSInteger)page {
