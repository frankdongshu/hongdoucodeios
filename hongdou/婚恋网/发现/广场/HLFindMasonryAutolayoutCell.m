...
@interface HLFindMasonryAutolayoutCell ()<TZImagePickerControllerDelegate>
...
@implementation HLFindMasonryAutolayoutCell
...
- (void)setAlbumModel:(HLAlbumDetails *)albumModel{
    ...
    if (kISNullObject(albumModel)) {
        return;
    }
    
    if ([self.albumModel.member isEqualToString:@"0"]) { // 非会员
    ...
    if (albumModel.photoArray.count>0) {
        ...
        int num = albumModel.photoArray.count == 7 ? 2:1;
        ...
        [self.photosView mas_updateConstraints:^(MASConstraintMaker *make) {
            ...
            make.height.mas_equalTo((albumModel.photoArray.count/4 +num) * width + albumModel.photoArray.count/4*(10*num));
...
-(void)setUpAllView{
    ...
    // vip
    self.vipImgV = [UIImageView zj_imageViewWithImage:@"icon_vip" SuperView:self.contentView contentMode:UIViewContentModeScaleAspectFill isClip:YES constraints:^(MASConstraintMaker *make) {
