...
- (void)awakeFromNib {
    ...
    self.shiRenLab.layer.cornerRadius = 10.f;
    self.shiRenLab.layer.masksToBounds = YES;
    
    self.renLianLab.layer.cornerRadius = 10.f;
    self.renLianLab.layer.masksToBounds = YES;
...
- (void)setModel:(HLUser *)model{
    ...
    self.ageLable.text = @"";
    if (model.age.length>0 && model.habitation.length>0) {
        self.ageLable.text = [NSString stringWithFormat:@" %@岁· %@ ",model.age,model.habitation];
    } else {
    ...
    if (!kISNullObject(model.constellation)) {
        
        self.constellaLabel.hidden = NO;
        
        NSMutableAttributedString *text = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@" %@ ",model.constellation]];
    ...
    //    self.constellaLabel.text =  [NSString stringWithFormat:@" %@ ", model.constellation.length ? model.constellation : @""];
        ...
        self.constellaLabel.hidden = YES;
    }
    
    self.shiRenLab.hidden = NO;
    self.renLianLab.hidden = NO;
    
    // 实人认证
    if (![model.identification isEqualToString:@"1"]) {
        
        // 人脸认证
        if (![model.certification isEqualToString:@"1"]) {
            self.shiRenLab.hidden = YES;
            self.renLianLab.hidden = YES;
        } else {
            
            self.renLianLab.hidden = YES;
            self.shiRenLab.text = @" 人脸认证 ";
            self.shiRenLab.backgroundColor = kRGB(241, 179, 150);
        }
        
        
    } else {
        self.shiRenLab.text = @" 实人认证 ";
        self.shiRenLab.backgroundColor = kRGB(168, 203, 75);
        
        // 人脸认证
        if (![model.certification isEqualToString:@"1"]) {
            self.renLianLab.hidden = YES;
        } else {
            self.renLianLab.text = @" 人脸认证 ";
        }
    ...
    self.addLabel.text = model.height.length ? [NSString stringWithFormat:@"%@",model.height] :@"";
    self.stageLable.text = model.education.length ? [NSString stringWithFormat:@"  %@",model.education] : @"";
    self.occupationLabel.text = model.position.length ? [NSString stringWithFormat:@"  %@",model.position] : @"";
    self.incomeLabel.text = model.earns.length ? [NSString stringWithFormat:@"  %@",model.earns] : @"";
...
//    [self.isLikeButton setTitle:model.fans.length ? model.fans : @"0" forState:UIControlStateNormal];
    //修改因此关注数量
    [self.isLikeButton setTitle:model.fans.length ?@"" : @"" forState:UIControlStateNormal];
    ...
    if (buttonIndex == 1) {
        ...
        if (alertView.tag == 110) {
            ...
            NSDictionary *params = @{
                ...
                @"sid":self.model.userid
            };
            
            WeakSelf(weakSelf);
            [HLHTTPSessionManager postDataWithNSString:HLGoPrivacy_Shield withDictionary:params success:^(NSDictionary * _Nonnull dictionary) {
...
- (IBAction)followButtonClick:(id)sender {
    ...
    if (self.isLikeButton.selected) { // 取消关注
