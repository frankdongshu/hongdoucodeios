...
@interface LLBugVipController ()<UITableViewDelegate, UITableViewDataSource, LLBuyVipCellDelegate> {
...
@implementation LLBugVipController
...
//    [self forbiddenGesture];
    ...
    self.sc_navigationBar.title = @"购买信息置顶";
...
- (void)didSelectBuyButtonWithProductID:(NSString *)productID {
    ...
    [_IAPTool startIAPWithProductID:productID agentId:nil  andUrl:@"/mind/newranking" completeHandle:^(IAPResultType type, NSData * _Nonnull data) {
...
- (void)requestVipList {
    ...
    [HTTPSessionManger postDataWithNSString:@"/coach/member_price" withDictionary:parmas success:^(NSDictionary * _Nonnull dictionary) {
        ...
        NSLog(@"/coach/member_price: %@",dictionary);
...
- (void)getMemberDate {
    ...
    [self.view showLoading];
    [HLHTTPSessionManager postDataWithNSString:@"/mind/is_newranking" withDictionary:parmas success:^(NSDictionary * _Nonnull dictionary) {
        [self.view hideLoading];
        
        NSLog(@"/mind/is_newranking: %@",dictionary);
        
        if ([[dictionary[@"code"] stringValue] isEqualToString:@"200"]) {
            
            self.headerView.typeLabel.attributedText = [[NSString alloc] attriStringWithFirstString:@"您的信息置顶功能到期日是：" withTwoString:[NSString stringWithFormat:@"%@",!kISNullObject(dictionary[@"data"][@"date"]) ? dictionary[@"data"][@"date"] : @"未开通"] withThreeString:@"" color:[UIColor blackColor]];
