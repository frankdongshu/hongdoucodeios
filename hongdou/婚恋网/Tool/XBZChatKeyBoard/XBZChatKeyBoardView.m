...
@interface XBZChatKeyBoardView () <UITextViewDelegate, XBZChatEmojiViewDelegate, XBZChatMoreViewDelegate,XBZKeyBoardGreetListViewDelegate>
...
// 问候句子列表
@property (nonatomic, assign) XBZKeyBoardGreetListView *greetView;
...
//        self.layer.borderWidth = 1.f;
        ...
        layer.backgroundColor = [UIColor colorWithRGB:0xe0e0e0].CGColor;
...
- (void)layoutSubviews {
    ...
    // 只支持表情
//    self.textView.frame = CGRectMake(kHorizenSpace, kVerticalSpace-2.f, SCREEN_Width-(3*kHorizenSpace)-(1*kButtonHeight), self.topBarView.height-2*(kVerticalSpace-2.f));
    ...
    if (!self.bottomView.hidden) {
        ...
        if (!_isChating) {
            self.greetView.frame = self.bottomView.bounds;
        }else{
...
- (void)textViewDidBeginEditing:(UITextView *)textView {
    if (!_isChating) {
        ...
        self.currentState = XBZKeyBoardStateGreet;
        if (![self.bottomView.subviews containsObject:self.greetView]) {
            [self.bottomView addSubview:self.greetView];
        }
        return;
    }
    ...
    if (!_isSending) {
        [self endEditing:YES];
        [self.delegate openBuyVipViewWithMessage];
        return;
    }
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text {
    if ([text isEqualToString:@""]) {
        ...
        if ([self.delegate respondsToSelector:@selector(chatKeyBoardViewSendTextMessage:originText:)]) {
            ...
            [self.delegate chatBarFrameDidChangeFrame:self.frame];
        }
...
- (void)chatGreetDidSelectItemWithTitle:(NSString *)title index:(NSInteger)index{
    if (self.delegate && [self.delegate respondsToSelector:@selector(chatDidSelectGreetImteTitle:index:)]) {
        [self.delegate chatDidSelectGreetImteTitle:title index:index];
    }
}

#pragma mark  跳转语句设置界面
- (void)skipGreetingsStatementVC {
    
    [self.delegate pushYuJuVC];
    
}

- (void)skipOpenVipVC {
    
    [self.delegate pushOpenVipVC];
}
...
- (void)setIsChating:(BOOL)isChating{
...
- (void)setCurrentState:(XBZKeyBoardState)currentState {
    ...
    switch (currentState) {
        ...
        case XBZKeyBoardStateFace:
        {
            ...
            [self showBottomViewWithDuration:0.15f];
        }
            break;
        case XBZKeyBoardStateMore:
        {
...
- (XBZKeyBoardGreetListView *)greetView{
    if (!_greetView) {
        _greetView = [[XBZKeyBoardGreetListView alloc] initWithFrame:CGRectMake(0, 0, self.width, kBottomViewHeight)];
        _greetView.delegate = self;
    }
    return _greetView;
}
