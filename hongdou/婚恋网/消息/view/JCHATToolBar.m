...
- (void)switchToVoiceInputMode {
  ...
  [self.voiceButton setImage:[UIImage imageNamed:@"icon_keybord"] forState:UIControlStateNormal];
...
- (void)drawRect:(CGRect)rect {
  if (self.startRecordButton){
    ...
    [self.startRecordButton mas_makeConstraints:^(MASConstraintMaker *make) {
      ...
      make.left.mas_equalTo(self.voiceButton.mas_right).with.offset(6);
      make.right.mas_equalTo(self.enmojoyButton.mas_left).with.offset(-6);
    }];
    return;
  }
  ...
  [self setFrame:CGRectMake(0, kScreenHeight - kTabBarHeight, self.bounds.size.width, 49)];
  
  //    self.startRecordButton = [UIButton buttonWithType:UIButtonTypeRoundedRect];
  self.startRecordButton = [UIButton new];
  
  [self.startRecordButton setTitleColor:[UIColor colorWithHex:0xB4C2CE] forState:UIControlStateNormal];
  [self.startRecordButton setTitleColor: [UIColor colorWithHex:0xB4C2CE] forState:UIControlStateHighlighted];
  ...
  [self.startRecordButton setBackgroundColor:[UIColor colorWithHex:0xffffff]];
  [self.startRecordButton addTarget:self action:@selector(holdDownButtonTouchDown) forControlEvents:UIControlEventTouchDown];
    self.startRecordButton.layer.cornerRadius = 4.f;
    self.startRecordButton.layer.borderWidth = 0.8f;
    self.startRecordButton.layer.borderColor = [UIColor colorWithHex:0xB4C2CE].CGColor;
  ...
  self.recordAnimationView=[[JCHATRecordAnimationView alloc]initWithFrame:CGRectMake((kScreenWidth-140)/2, (kScreenHeight -kNavigationBarHeight - kTabBarHeight - 140)/2, 140, 140)];
...
- (void)adjustTextViewHeightBy:(CGFloat)changeInHeight {
  ...
  CGSize textSize = [JCHATStringUtils stringSizeWithWidthString:_textView.text withWidthLimit:_textView.frame.size.width withFont:[UIFont systemFontOfSize:17.f]];
  CGFloat textViewHeight = textSize.height + 40;
  _textViewHeight.constant = textViewHeight>39 ? textViewHeight:40;
...
+ (CGFloat)textViewLineHeight {
  return 17.f * [UIScreen mainScreen].scale; // for fontSize 16.0f
}
...
- (void)addtoolbar {
  self.toolbar.frame = CGRectMake(0, 0, kScreenWidth, kTabBarHeight - kScreenBottomMargin);
