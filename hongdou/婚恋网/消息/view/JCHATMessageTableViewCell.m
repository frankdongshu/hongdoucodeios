...
#define kVoiceTimeLabelHidenFrame [_model.message isReceived]?CGRectMake(_messageContent.frame.origin.x + _messageContent.frame.size.width + 5, _messageContent.frame.size.height/2 - 8, 30, 17):CGRectMake(_messageContent.frame.origin.x, _messageContent.frame.size.height/2 - 8, 30, 17)
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    ...
    if ([super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
      ...
      [_headView setImage:[UIImage imageNamed:@"icon_head"]];
      ...
      _voiceTimeLabel.font = [UIFont systemFontOfSize:16];
        _voiceTimeLabel.textColor = [UIColor colorWithHex:0x333333];
        ...
        if (error == nil) {
            ...
            if ([objectId isEqualToString:user.username]) {
                if (data != nil) {
                    ...
                    [self.headView setImage:[UIImage imageNamed:@"icon_head"]];
                }
            }
        } else {
            [self.headView setImage:[UIImage imageNamed:@"icon_head"]];
      ...
      if ([strongSelfUpload.model.message.msgId isEqualToString:msgId]) {
        ...
        strongSelfUpload.percentLabel.text = percentString;
          if ([percentString isEqualToString:@"100%"]) {
              strongSelfUpload.percentLabel.hidden = YES;
          }
      }
    });
  };
...
//    [_messageContent setBubbleSide:isRecive];
    ...
    [_headView setFrame:CGRectMake(kScreenWidth - headHeight - gapWidth, 0, headHeight, headHeight)];//头像位置
//    [_messageContent setBubbleSide:isRecive];
    [_messageContent setFrame:CGRectMake(kScreenWidth - headHeight - 5 - contentSize.width, 0, contentSize.width, contentSize.height)];
...
- (void)tapContent:(UIGestureRecognizer *)gesture {
  ...
  if (_model.message.contentType == kJMSGContentTypeImage) {
    ...
    if (_model.message.contentType == kJMSGContentTypeLocation) {
        if (self.delegate && [self.delegate respondsToSelector:@selector(tapLoction:tapView:tableViewCell:)]) {
            [self.delegate tapLoction:_indexPath tapView:(UIImageView *)gesture.view tableViewCell:self];
        }
    }
