...
@interface HLMeFollowsViewController ()<HLPrivacyManageDeleagte>
{
    UIAlertView * alert;
...
@implementation HLMeFollowsViewController

- (HLNotVipView *)notView {
    if (!_notView) {
        _notView = [[HLNotVipView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, kScreenHeight) andDataArray:self.dataSource andTitle:@"我关注"];
...
- (void)loadNewData{
    WeakSelf(weakSelf);
    [HLHTTPSessionManager postDataWithNSString:HLMyFollow_Shields withDictionary:@{@"uid":[LoginManager defaultManager].userid} success:^(NSDictionary * _Nonnull dictionary) {
...
- (void)deleteButtonClickIndexPath:(NSIndexPath *)indexPath{
    ...
    alert = [[UIAlertView alloc] initWithTitle:@"提示" message:@"确定不在关注？" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
    alert.tag = 110;
    [alert show];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (buttonIndex == 1) {
        if (alertView.tag == 110){
            ...
            [HLHTTPSessionManager postDataWithNSString:HLCancelFollow_Shields withDictionary:@{@"uid":[LoginManager defaultManager].userid,@"fid":model.userid} success:^(NSDictionary * _Nonnull dictionary) {
