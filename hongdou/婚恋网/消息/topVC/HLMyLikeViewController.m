...
#import "HLMyLikeViewController.h"
...
@interface HLMyLikeViewController ()
...
@implementation HLMyLikeViewController

- (HLNotVipView *)notView {
    if (!_notView) {
        _notView = [[HLNotVipView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, kScreenHeight) andDataArray:self.dataSource andTitle:@"我赞过"];
...
- (void)loadNewData{
    WeakSelf(weakSelf);
    [HLHTTPSessionManager postDataWithNSString:HLMyLike withDictionary:@{@"uid":[LoginManager defaultManager].userid} success:^(NSDictionary * _Nonnull dictionary) {
