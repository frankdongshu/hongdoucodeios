...
@interface HLSeenMeViewController ()
...
@implementation HLSeenMeViewController

- (HLNotVipView *)notView {
    if (!_notView) {
        _notView = [[HLNotVipView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, kScreenHeight) andDataArray:self.dataSource andTitle:@"看过你"];
...
- (void)loadNewData{
    WeakSelf(weakSelf);
    [HLHTTPSessionManager postDataWithNSString:HLSeenMy_Shields withDictionary:@{@"uid":[LoginManager defaultManager].userid} success:^(NSDictionary * _Nonnull dictionary) {
