...
@interface HLMeSeensViewController ()
...
@implementation HLMeSeensViewController

- (HLNotVipView *)notView {
    if (!_notView) {
        _notView = [[HLNotVipView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, kScreenHeight) andDataArray:self.dataSource andTitle:@"我看过"];
...
- (void)loadNewData{
    WeakSelf(weakSelf);
    [HLHTTPSessionManager postDataWithNSString:HLMySeen_Shields withDictionary:@{@"uid":[LoginManager defaultManager].userid} success:^(NSDictionary * _Nonnull dictionary) {
