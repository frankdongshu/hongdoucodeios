...
-(instancetype)init {
    ...
    self = [self initWithFrame:CGRectMake(0, kScreenHeight -chatBarHeight -kSafeAreaBottom, kScreenWidth, chatBarHeight +kSafeAreaBottom)];
...
-(void)showFaceView:(BOOL)show {
    ...
    if (show) {
        ...
        [self setFrame:CGRectMake(0, self.superViewHeight -chatBarFaceHeight -chatBarHeight -kSafeAreaBottom, self.frame.size.width, chatBarHeight) animated:YES];
    }else {
...
-(void)showMoreView:(BOOL)show {
    ...
    if (show) {
        ...
        [self setFrame:CGRectMake(0, self.superViewHeight -chatBarMoreHeight -chatBarHeight -kSafeAreaBottom, self.frame.size.width, chatBarHeight) animated:YES];
    }else {
...
-(void)endInputing {
    ...
    [self showMoreView:NO];
    [self showFaceView:NO];
    [self setFrame:CGRectMake(0, self.superViewHeight -chatBarHeight -kSafeAreaBottom, self.frame.size.width, chatBarHeight) animated:YES];//输入框移到底部
}
...
-(CGFloat)bottomHeight {
    ...
    if (self.chatmoreView.superview) {
        return MAX(self.keyboardFrame.size.height, self.chatmoreView.frame.size.height +kSafeAreaBottom);
    }else{
        return MAX(self.keyboardFrame.size.height, CGFLOAT_MIN +kSafeAreaBottom);
    }
}
...
-(ZPHTextView *)textView {
    ...
    if (!_textView) {
        ...
        _textView = [[ZPHTextView alloc]initWithFrame:CGRectMake(5, 2, self.bounds.size.width -10, self.bounds.size.height -32 -kSafeAreaBottom)];
...
-(ZPHChatLabelling *)backView {
    ...
    if (!_backView) {
        ...
        _backView = [[ZPHChatLabelling alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(_textView.frame), self.bounds.size.width, 32 +kSafeAreaBottom)];
