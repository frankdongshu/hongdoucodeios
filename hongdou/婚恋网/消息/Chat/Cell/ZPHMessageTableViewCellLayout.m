...
#import "ZPHMessageTableViewCellLayoutCard.h"
...
-(instancetype)initWithDictionary:(NSDictionary *)dictionary {
    ...
    if (self = [super init]) {
        ...
        if (!kISNullObject(dictionary[@"type"])) { // 接收的是type
            
            if ([dictionary[@"type"] isEqualToString:@"text"]) {
                self = [[ZPHMessageTableViewCellLayoutText alloc] initWithDictionary:dictionary];
            }
            else if ([dictionary[@"type"] isEqualToString:@"pic"]) {
                self = [[ZPHMessageTableViewCellLayoutImage alloc] initWithDictionary:dictionary];
            }
            else if ([dictionary[@"type"] isEqualToString:@"voi"]) {
                self = [[ZPHMessageTableViewCellLayoutVoice alloc] initWithDictionary:dictionary];
            }
            else if ([dictionary[@"type"] isEqualToString:@"card"]) {
                self = [[ZPHMessageTableViewCellLayoutCard alloc] initWithDictionary:dictionary];
            }
            else if ([dictionary[@"type"] isEqualToString:@"html"]) {
                self = [[ZPHMessageTableViewCellLayoutHtml alloc] initWithDictionary:dictionary];
            }
            else {
                // 未知类型暂时为文字
                self = [[ZPHMessageTableViewCellLayoutText alloc] initWithDictionary:dictionary];
        ...
        } else if (!kISNullObject(dictionary[@"cut"])) { // 发送是传的是cut
            
            if ([dictionary[@"cut"] isEqualToString:@"text"]) {
                self = [[ZPHMessageTableViewCellLayoutText alloc] initWithDictionary:dictionary];
            }
            else if ([dictionary[@"cut"] isEqualToString:@"pic"]) {
                self = [[ZPHMessageTableViewCellLayoutImage alloc] initWithDictionary:dictionary];
            }
            else if ([dictionary[@"cut"] isEqualToString:@"voi"]) {
                self = [[ZPHMessageTableViewCellLayoutVoice alloc] initWithDictionary:dictionary];
            }
            else if ([dictionary[@"type"] isEqualToString:@"card"]) {
                self = [[ZPHMessageTableViewCellLayoutCard alloc] initWithDictionary:dictionary];
            }
            else if ([dictionary[@"cut"] isEqualToString:@"html"]) {
                self = [[ZPHMessageTableViewCellLayoutHtml alloc] initWithDictionary:dictionary];
            }
            else {
                // 未知类型暂时为文字
...
- (double)getTimeStrWithString:(NSString *)str{
...
//        NSString *timeString = [self messageTimeWithModelTime:[NSString stringWithFormat:@"%@",model.time]];
        ...
        NSString *timeString = [JCHATStringUtils getFriendlyDateString:[model.time doubleValue]];
        
        NSString *ontimeString = [JCHATStringUtils getFriendlyDateString:[model.ontime doubleValue]];
        
        if ([timeString isEqualToString:ontimeString]) {
