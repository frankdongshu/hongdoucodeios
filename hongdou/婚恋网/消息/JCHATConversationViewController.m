...
//#import "JCHATPersonViewController.h" // 个人信息
//#import "JCHATFriendDetailViewController.h"  // 详情
...
- (void)requestChatting{
    ...
    [HLHTTPSessionManager postDataWithNSString:HLIS_Chatting withDictionary:@{@"uid":[LoginManager defaultManager].userid,@"mobile":self.userName} success:^(NSDictionary * _Nonnull dictionary) {
        ...
        if ([code isEqualToString:@"200"] ) {
            self->isChatting = [[dictionary[@"data"] objectForKey:@"is"] boolValue];
            ...
            if (isChatting) {
                ...
                [weakSelf.messageTableView mas_updateConstraints:^(MASConstraintMaker *make) {
                    ...
                    make.height.mas_equalTo(kScreenHeight - kNavigationBarHeight - kTabBarHeight);
                }];
            }else{
...
- (void)viewDidLayoutSubviews {
    ...
    [self.toolBarContainer mas_updateConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.messageTableView.mas_bottom);
    }];
}
...
- (void)setupComponentView {
    ...
    self.openChatView.frame = CGRectMake(0,kNavigationBarHeight,kScreenWidth,44);
    ...
    UILabel *titleLable = [[UILabel alloc] initWithFrame:CGRectMake(20, 0, 150, 44)];
    titleLable.text = @"想了解更多的她/他吗？";
    titleLable.textColor = [UIColor whiteColor];
    titleLable.font = [UIFont systemFontOfSize:13.f];
    [self.openChatView addSubview:titleLable];
    
    UIButton *btn = [[UIButton alloc] initWithFrame:CGRectMake(kScreenWidth - 120, 4, 100, 36)];
    btn.backgroundColor = [UIColor colorWithHex:0xFFCA10];
    [btn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [btn setTitle:@"立即开通畅聊" forState:UIControlStateNormal];
...
//    [self.view addSubview:_messageTableView];
    ...
    [_messageTableView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.equalTo(self.view.mas_top).offset(kNavigationBarHeight + 44);
        make.height.mas_equalTo(kScreenHeight - kNavigationBarHeight - kTabBarHeight - 44);
    }];
...
//
//    self.toolBarContainer   =  [[JCHATToolBarContainer alloc] init];
//
//
...
//        make.left.right.equalTo(self.view);
//        make.bottom.equalTo(self.view.mas_bottom).offset(kScreenBottomMargin);
//        make.height.mas_equalTo(kTabBarHeight - kScreenBottomMargin);
//    }];
    self.toolBarHeightConstrait.constant = 49.f;
    self.toolBarToBottomConstrait.constant = kScreenBottomMargin;
...
//    [self.moreViewContainer mas_makeConstraints:^(MASConstraintMaker *make) {
//        make.left.right.equalTo(self.view);
//        make.bottom.equalTo(self.view.mas_bottom).offset(kScreenBottomMargin);
//        make.height.mas_equalTo(0);
//    }];
    self.moreViewHeight.constant = 0.f;
   _moreViewContainer.moreView.delegate = self;
   _moreViewContainer.moreView.backgroundColor = [UIColor whiteColor];
}
...
- (void)inputKeyboardWillShow:(NSNotification *)notification{
  ...
  [UIView animateWithDuration:animationTime animations:^{
      self->_moreViewHeight.constant = keyBoardFrame.size.height;
...
- (void)inputKeyboardWillHide:(NSNotification *)notification {
  CGFloat animationTime = [[[notification userInfo] objectForKey:UIKeyboardAnimationDurationUserInfoKey] floatValue];
  [UIView animateWithDuration:animationTime animations:^{
      self->_moreViewHeight.constant = 0;
    [self.view layoutIfNeeded];
  }];
  [self scrollToBottomAnimated:NO];
}
...
- (void)perform {
  ...
  _toolBarToBottomConstrait.constant = kScreenBottomMargin;
}

#pragma mark --返回下面的位置
- (void)dropToolBar {
  _barBottomFlag =YES;
  _previousTextViewContentHeight = 49;
...
- (void)pressMoreBtnClick:(UIButton *)btn {
  ...
  _toolBarToBottomConstrait.constant = kScreenBottomMargin;
  _moreViewHeight.constant = 227+kScreenBottomMargin;
  [_messageTableView setNeedsDisplay];
  [_moreViewContainer setNeedsLayout];
  [_toolBarContainer setNeedsLayout];
  [UIView animateWithDuration:0.25 animations:^{
      self->_toolBarToBottomConstrait.constant = 0;
      self->_moreViewHeight.constant = 227 + kScreenBottomMargin;
      
      [self->_messageTableView layoutIfNeeded];
      [self->_toolBarContainer layoutIfNeeded];
      [self->_moreViewContainer layoutIfNeeded];
  }];
...
- (void)prepareTextMessage:(NSString *)text {
    ...
    NSLog(@"Action - prepareTextMessage");
...
//        NSLog(@"1.index %ld beyond bounds %ld",indexPath.row,_allmessageIdArr.count);
        return 40;
    }
...
- (void)finishRecorded {
  ...
  [self.voiceRecordHelper stopRecordingWithStopRecorderCompletion:^{
    [weakSelf SendMessageWithVoice:weakSelf.voiceRecordHelper.recordPath
...
- (void)layoutAndAnimateMessageInputTextView:(UITextView *)textView {
  ...
  if (changeInHeight != 0.0f) {
    ...
    CGSize textSize = [JCHATStringUtils stringSizeWithWidthString:textview.text withWidthLimit:textView.frame.size.width withFont:[UIFont systemFontOfSize:17.f]];
...
- (void)showPopSelector{
    ...
    s.option_optionContents = @[@"修改昵称备注",@"聊天置顶",@"清空聊天记录", @"拉黑并举报", @"拉黑"];
...
//拉黑
- (void)pushBlack{
//    [HLHTTPSessionManager postDataWithNSString:HLPull_Black withDictionary:@{@"uid":[LoginManager defaultManager].userid,@"bid":@"",@"mobile":@""} success:^(NSDictionary * _Nonnull dictionary) {
//        NSString *code = [NSString stringWithFormat:@"%@",[dictionary objectForKey:@"code"]];
//        if ([code isEqualToString:@"200"] ) {
//            [self.navigationController popViewControllerAnimated:YES];
//
...
//    comPlaintVC.userInfo = self.userInfo;
